...
| Public tree | Cap 150 nodes, 200 chars text; slim `frame: [x,y,w,h]`; raw `bounds` stay runtime-only | `atspi.public_tree` / `slim_node` |
| Pixel clicks | Require a prior screenshot; last-shot meta maps scale/crop/origin/monitor scale | `PIXEL_SHOT_NEEDED`, tests |
| Cursor | Hyprland `hl.dsp.cursor.move` **teleport** (one dispatch + 30 ms settle). Drag interpolates while the button is held. | `input.movecursor` |
| Click / scroll / drag | ydotool uinput; **never** `ydotool --absolute`. Click prefers AT-SPI DoAction, then cached bounds center, then pixels. | `input.py` `click`/`scroll`/`drag`; `UseComputerTool` |
...
| Wait | `time.sleep`, cap 5 s, then harness tree observe | `WAIT_CAP_MS` |
| Observe after act | Default `observe: tree` after click/type/key/scroll/drag; JPEG iff `tree_usable` is false | `_attach_observe` |
| Type follow-up | `focused_from_nodes` on the last/new snapshot — **no** `focused_element()` re-walk | `computer_use.py` `action == "type"` |
...
| Session lifetime | Held until Stop / lock / error / `end_computer_use`. Assistant `done` leaves the grant; QML `grantedIdle` unlocks the pointer. | `Ai.qml`, `ComputerUse.markAgentIdle` |
...
- [x] Inline computer-use skill into the system prompt when CU is on;
      remove “Read the computer-use skill for details”
- [x] While CU session is active, map or reject native `screenshot`
      (aliased to grim `use_computer action=screenshot`)
- [x] After click/type/key/scroll/drag, return tree (and JPEG iff
      `tree_usable` is false) without a second model call
- [x] On `PIXEL_SHOT_NEEDED`, include `"next": {"action":"screenshot"}`
      (and auto-capture when grim works)
- [x] Auto-screenshot when snapshot `tree_usable` is false
- [x] Click `element_index` via cached bounds if `DoAction` missing
- [x] Stop `focused_element()` after `type`
- [x] Teleport cursor (drop targeting ease); drag still interpolates
- [x] Give slim nodes a `frame: [x,y,w,h]` (runtime already has bounds)
- [x] Unique Gemini function-call ids; move screenshot `inline_data`
      off the function role onto a follow-up user turn
- [x] Real schemas for `get_windows` / `focus_window`

**C. Session / tokens**

- [x] Do not end the CU grant on assistant `done`; end on Stop / lock /
      error / `end_computer_use`. Idle grant unlocks the pointer
      (`grantedIdle`) and exclusive keys; next mutate re-locks.
- [x] Update computer-use skill HUD paragraph to match (no
      “request again for another pass”)
- [x] Honor ordered `actions[]` even when top-level `action` is set
- [x] Shrink doctor payload to blockers + `can_*` (no window dump)
...
**Deferred (tracked, not Linux/Hyprland blockers)**

- Zoom / region capture that preserves full-screenshot click space (P1.7)
- `get_app_state` single QML hop (P1.8)
- Cache session flags in Python to skip per-keystroke native `gate` (P1.9)
- Wait-until tree/pixel hash (P1.10) — wait still sleeps then observes
- grim JPEG / skip double ImageMagick (P1.11)
- Browser CDP path (P2.13)
- Virtual pointer / `zwlr_virtual_pointer` (P2.14)
- App allowlist (P2.15)
- Anthropic `computer_toolset_20260801` (P2.16)
- axctl cursor/shortcut for Niri/Mango (P2.17) — skill documents Hyprland-only
- IME / text-input-v3 (P2.18)
- Screenshot prompt-injection classifier (P3.19)
- macOS AX (not this runtime)

### Tiny fix landed in the audit pass

`assets/ai/skills/screenshot/SKILL.md` no longer claims
`action=snapshot` is silent grim. The implementation pass also aliases
native `screenshot` during an active CU session.
