...
# === Configuration ===
REPO_URL="https://github.com/git-napkin/ambxst+-plus.git"
...
install_dependencies() {
  case "$DISTRO" in
  nixos)
    local FLAKE_URI="${1:-github:git-napkin/ambxst+-plus}"
  ...
  fedora)
    ...
    local PKGS=(
      kitty tmux fuzzel network-manager-applet
  ...
  arch)
    ...
    local PKGS=(
      kitty tmux fuzzel network-manager-applet
      pipewire wireplumber easyeffects ffmpeg x264 playerctl
      ...
      matugen gpu-screen-recorder wf-recorder wl-clip-persist gradia
