...
Item {
    ...
    NumberAnimation on phase {
        running: (root.dashedActive || root.wavy) && root.visible && root.opacity > 0 && (!root.Window.window || root.Window.window.visible)
        from: 0
    ...
    Timer {
        id: waveTimer
        interval: 32 // ~30 FPS
        running: root.wavy && root.visible && root.opacity > 0 && (!root.Window.window || root.Window.window.visible) && (root.value > 0 || root.isDragging)
        repeat: true
        onTriggered: {
